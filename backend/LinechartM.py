...
def insert_signal_values(connection, value):
    try:
        ...
        sql = """
        INSERT INTO Linechart (logical_address, address_ip, signal_value, date_takeen, type_date)
    VALUES (%s, %s, %s, %s, 'month') AS new_val
...
def select_signals(connection):
    try:
        cursor = connection.cursor()
        sql = """
        SELECT d.logical_address, d.address_ip, ds.signal_value
        FROM dashboard d
        ...
        WHERE dg.group_name = "Line_chart";
        """
        cursor.execute(sql)
...
def add_time_to_rows(rows):
    now = datetime.now()
    time_without_seconds_and_minutes = now.strftime("%Y-%m-%d")
